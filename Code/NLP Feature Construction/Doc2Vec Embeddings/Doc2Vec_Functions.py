...
def run_doc2vec(sample_run, train_set_only):
    ...
    if sample_run:
        ...
        # read in 100 rows from the first file
        df = pd.read_parquet(r'../../../Data/All_Data/All_Data_with_NLP_Features/' + file_list[0], columns=['ticker', 'fixed_quarter_date', 'transcript', 'train_test_80_20']).head(100)
    else:
        # list of files in '../../../Data/All_Data/All_Data_with_NLP_Features'
        file_list = [f for f in os.listdir(r'../../../Data/All_Data/All_Data_with_NLP_Features') if f.endswith('.parquet')]
        # read in all parquet files
        df = pd.concat([pd.read_parquet(r'../../../Data/All_Data/All_Data_with_NLP_Features/' + f, columns=['ticker', 'fixed_quarter_date', 'transcript', 'train_test_80_20']) for f in file_list])
    print('dataframe')
    print(df.head())
    ...
    if not sample_run:
        if train_set_only:
            ...
            df.to_parquet(r'../../../Data/Doc2Vec_Vectors/all_data_doc2vec_vectors.parquet')
    ...
    # Note sample is fraction of full roughly 8K rows
    if sample_run:
        print('Estimated time on full dataset:', (8000 / len(df)) * ((end_time - start_time) / 60), 'minutes')
